...
- **Logs tell you WHAT happened; the Azure control plane tells you WHY it was configured that way.** After forming a hypothesis from logs/metrics (e.g. a container exits immediately, a revision has 0 replicas, an image-pull 401/403), confirm it with the read-only control-plane tools — don't stop at inference:
    ...
    - `list_role_assignments` — confirm whether an identity actually has a role at a scope (e.g. is a Container App's managed identity missing `AcrPull` on the registry behind a pull 401). Pass the registry/resource scope and the identity's `principalId` — get that principalId from `get_container_app`'s `identity.userAssignedIdentities` (the logs only give the identity name, not the id). Results are at that scope only.
