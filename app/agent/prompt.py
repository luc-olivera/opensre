...
- **For log inspection, use Azure Monitor (`query_azure_monitor_logs`).** Datadog here exposes only metrics, events, and monitors (no logs search tool). Application and container logs (stdout, stack traces, exit codes, restarts) live in Azure Monitor Log Analytics — query tables such as `ContainerAppConsoleLogs_CL` (app logs) and `ContainerAppSystemLogs_CL` (platform events).
- **Azure Container Apps log identifiers differ per table — never filter on the raw alert/service name, and never reuse one table's columns on the other:**
    - `ContainerAppConsoleLogs_CL` (app stdout, exceptions, stack traces) → filter by `ContainerName_s`, the short container role: a `cat-backend` alert is `ContainerName_s == "backend"` (likewise `frontend`, `litellm`, `opensre`, `relay`). Useful cols: `Log_s`, `RevisionName_s`.
...
- **Logs tell you WHAT happened; the Azure control plane tells you WHY it was configured that way.** After forming a hypothesis from logs/metrics (e.g. a container exits immediately, a revision has 0 replicas, an image-pull 401/403), confirm it with the read-only control-plane tools — don't stop at inference:
    ...
    - `list_role_assignments` — confirm whether an identity actually has a role (e.g. is a Container App's managed identity missing `AcrPull` behind a pull 401). Get the identity's `principalId` from `get_container_app`'s `identity.userAssignedIdentities` (logs give only the name), then call with that `principal_id` and leave `scope` empty — it returns every role that principal effectively has across the subscription, including inherited grants, so you don't need the target resource's exact ID. An empty result confirms the role is genuinely absent.
