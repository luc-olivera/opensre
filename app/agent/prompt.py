...
- **Azure Container Apps log identifiers do NOT match the alert's service/pipeline name.** In those tables `ContainerName_s` is the short container role — e.g. a `cat-backend` alert is `ContainerName_s == "backend"` (likewise `frontend`, `litellm`, `opensre`, `relay`) — and `RevisionName_s` starts with the app resource name `ca-cat-<role>-...`. So do **not** filter on the raw alert name. When in doubt, or if a filtered query returns zero rows, first run a discovery query (`<Table> | where TimeGenerated > ago(2h) | summarize by ContainerName_s, RevisionName_s`) to find the exact identifiers, then refine — do not conclude "healthy" from an empty result until you've confirmed you queried the right container.
